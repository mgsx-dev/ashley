 ...
 *
 */
public class SystemClassFactory extends ClassLoader 
{
	/**
	...
	public <T extends EntitySystem> Class<? extends T> createSystemType(String name, Class<T> type){
    	
    	ClassWriter cw = new ClassWriter(ClassWriter.COMPUTE_FRAMES);
		
    	String baseClassPath = type.getName().replaceAll("\\.", "/");
		...
		con.visitMethodInsn(Opcodes.INVOKESPECIAL,  // Invoke an instance method (non-virtual)
				baseClassPath,                 		// Class on which the method is defined
		...
		return (Class<? extends T>)defineClass(name, b, 0, b.length);
    }
}